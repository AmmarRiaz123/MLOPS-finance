...
def predict_direction_from_ohlcv(payload: Dict[str, Any], model_key: str = "lightgbm_up_down_model") -> Dict[str, Optional[float]]:
    """
    Predict direction using model-local feature builder.
    payload: OHLCV dict and optional 'history' list[dict].
    """
    if model_key != "lightgbm_up_down_model":
        raise RuntimeError(f"Unsupported model_key: {model_key}")

    training_folder = "lightgbm_up_down"
    ...
    return {"direction": str(label), "probability": probability}
