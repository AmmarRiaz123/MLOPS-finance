...
def evaluate():
    ...
    # create future dataframe for next 5 calendar days
    ...
    metrics = {}
    for horizon in (3,5):
        # target ds = last_train_date + horizon days
        target_ds = last_ds + pd.Timedelta(days=horizon)
    ...
    final_metrics = {
        ...
        "metrics": metrics,
        "train_end": str(last_ds.date())
    }
    with open(METRICS_LATEST, "w") as f:
        json.dump(final_metrics, f, indent=2)

    # save plot actual vs forecast (merge forecast and actual test series for range)
    # build series for plotting: history tail + forecast
    ...
    if 'yhat' in plot_df.columns:
        ...
        # use safe lower/upper arrays if columns exist
